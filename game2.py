...
def minMax(board, minOrMax, maxDepth, currentDepth=0):
    ...
    for i in range(0, 7):
        if minOrMax == "max":
            ...
            currentUtility = minMax(newBoard, currentDepth + 1, "min")
            arr.append(currentUtility)
        else:
            newBoard = insertPiece(board, i, 'B')
            currentUtility = minMax(newBoard, currentDepth + 1, ",max")
            arr.append(currentUtility)
...
def main():
    ...
    # b.board=insertPeice(b.board, 5, 'B')
    # b.print_grid()
    test()
    """
    while True:
